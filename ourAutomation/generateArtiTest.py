...
def genCaliperChannels(domainName, orgsCount, orderersCount, chaincodeName, peerCounts, chaincodeversion, chaincode_lang, chaincode_init_function, chaincode_path, orgNames, chaincode_init_arguments, chaincode_created):
    ...
    chConfig["contracts"] = [{"id" : chaincodeName,
               ...
               "initArguments" : chaincode_init_arguments,
               "created" : chaincode_created,
               "endorsementPolicy" : {'policy': {'{}-of'.format(int(len(orgNames)*70/100)): [{'signed-by': x} for x in range(len(orgNames))]},
               #"endorsementPolicy" : {'policy': {'1-of': [{'signed-by': x} for x in range(len(orgNames))]},
...
def genCaliperOrganizationsPerOrg(domainName, orgsCount, orderersCount, peerCounts, org):
    config = []
...
def getCaliperNetworkConfigPerOrg(domainName, orgsCount, orderersCount, chaincodeName, peerCounts, chaincodeversion, chaincode_lang, chaincode_init_function, chaincode_path, orgNames, chaincode_init_arguments, chaincode_created, endorsersList, org):
    ...
    caliperConfig["organizations"] = genCaliperOrganizationsPerOrg(domainName, orgsCount, orderersCount, peerCounts, org)

    fHandle = open("caliperNetworkConfig-{}.yaml".format(org), "w")
    stream = yaml.dump(caliperConfig, default_flow_style = False, sort_keys=False)
    fHandle.write(stream.replace("'", ""))
    #fHandle.write(stream)
    fHandle.close()
...
def getCaliperConnectionProfile(domainName, orgsCount, orderersCount, chaincodeName, peerCounts, chaincodeversion, chaincode_lang, chaincode_init_function, chaincode_path, org, ordererOrg, ordererIndexCounter):
    ...
    caliperConnectionProfile["name"] = "test-network-{}-net".format(org)
...
pathToBAF, chaincodeversion, chaincodeName, BAFChaincodePath, cloud_provider):
    ...
    for org in range(len(orgNames)):
        if orgNames[org] in endorsersList:
            ...
            'name': 'orderer{}'.format(ordCounter)} for ordCounter in range(orderersCount)]},
...
def genBAFOrderers(ordererOwnershipList, orderersCount, pathToBAF):
    config = []
    used = []
    for org in ordererOwnershipList:
        if org not in used:
            for ordCount in range(orderersCount):
                ordererConfig={'org_name': org, 'orderer': '', 'name': 'orderer{}'.format(ordCount),
                    'certificate': '{}/build/{}/orderer{}.crt'.format(pathToBAF, org, ordCount), 'type': 'orderer',
                    'uri': 'orderer{}.{}-net:7050'.format(ordCount, org)}
                config.append(ordererConfig)
            used.append(org)
    return config
...
def channelParticipantsList(orgNames, ordererOwnershipList, peerCounts, endorsersList):
    ...
    for org in range(len(orgNames)):
        if orgNames[org] in endorsersList:
            #a = [1,2,3]
            orgConfig = {'ordererAddress': 'orderer{}.{}-net:7050'.format(ordCount, ordererOwnershipList[org]),
...
def getBAFnetwork(domainName, orgsCount, orderersCount, chaincodeName, peerCounts, chaincodeversion, chaincode_lang, chaincode_init_function, chaincode_path, orgNames, BAFgit_protocol, BAFgit_url, BAFgitbranch, BAFgitrelease_dir, BAFgitchart_source, BAFgit_repo, BAFgitusername, BAFgitpassword, BAFgitemail, BAFgitprivate_key, BAFk8sContext, BAFk8sConfig_file, vaultUrl, vaultRootToken, endorsersList, ordererOwnershipList, pathToBAF, BAFChaincodePath, cloud_provider):
    ...
    'version': '2.2.0', 'orderers': genBAFOrderers(ordererOwnershipList, orderersCount, pathToBAF), 'env': {'retry_count': 50, 'type': '"local"', 'proxy': 'none',
...
def getCaliperValues(replicaMasterCount, replicaWorkersCount, repository, vaultUrl, vaultRootToken, endorsersList, peerCounts, orderersCount):
    ...
    'nodeSelector': {}, 'affinity': {}, 'nameOverride': '', 'tolerations': {'taint': 'caliper'}, 'imagePullSecrets': [], 'resources': {}, 'organization': {},
    'resultsFoldername':'{}-{}Orgs-{}Peers-{}Orderers'.format(datetime.datetime.fromtimestamp(ts).strftime('%Y%m%d%H%M%S'), len(endorsersList), '-'.join(str(i) for i in peerCounts), orderersCount)}
...
def generate():
    ...
    with open("fabricConfig.yaml", 'r') as stream:
        try:
            ...
            print(endorsersList, endorsersBooleanList, "peerCounts", peerCounts, "orderersCount",  ordererOwnershipList)

            orderersCount = len(endorsersList)
            print(orderersCount)
    ...
    for org in range(len(orgNames)):
        if orgNames[org] in endorsersList:
            ...
            getCaliperNetworkConfigPerOrg(domainName, orgsCount, orderersCount, chaincodeName, peerCounts, chaincodeversion, chaincode_lang, chaincode_init_function, chaincode_path, orgNames, chaincode_init_arguments, chaincode_created, endorsersList, orgNames[org])
    ...
    getCaliperValues(replicaMasterCount, replicaWorkersCount, caliperImageRepository, vaultUrl, vaultRootToken, endorsersList, peerCounts, orderersCount)
